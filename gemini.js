...
export async function analyzeFootballNews({title,description='',channel='',published_at='',url='',type='football feature'}={}){
  ...
  const response=await fetch(`${GEMINI_BASE}/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},body:JSON.stringify({contents:[{role:'user',parts:[{text:prompt}]}],tools:[{googleSearch:{}}],generationConfig:{temperature:0.35,maxOutputTokens:7000}})});
