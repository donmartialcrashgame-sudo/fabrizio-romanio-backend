...
import { registerGeminiRoutes, geminiConfigured } from './gemini.js';
import { registerMassiveRoutes, massiveConfigured } from './massive.js';
...
function getCredentialStatus() {
  return {
    ...
    watchmode_api_key: watchmodeConfigured(), tmdb_api_key: tmdbConfigured(), tmdb_read_access_token: Boolean(process.env.TMDB_API_READ_ACCESS_TOKEN), gemini_api_key: geminiConfigured(),
    massive_api_key: massiveConfigured()
  };
}
...
app.get('/',(_req,res)=>res.json({name:'Fabrizio Romano Backend',status:'ok',authentication:{supported:['Bearer Token','API Key/Secret','OAuth 2.0 Client ID/Secret','Access Token/Secret'],current_x_read_method:'Bearer Token',youtube_read_method:'API Key',watchmode_read_method:'X-API-Key header',tmdb_read_method:'Bearer token or API key',tmdb_user_lists:'TMDB v4 user access token',ai_news_analysis:'Gemini API with Google Search grounding',massive_market_data:'Massive REST API'},endpoints:['/health','/api/x-config','/api/x-test','/api/x-posts','/api/youtube-test','/api/youtube-videos','/api/watchmode-test','/api/movies','/api/movies/:id','/api/movies/:id/sources','/api/movie-genres','/api/movie-providers','/api/tmdb-test','/api/tmdb-movies','/api/tmdb-awards','/api/tmdb-movies/:id','/api/tmdb-movies/:id/providers','/api/tmdb-auth/start','/api/tmdb-auth/callback','/api/tmdb-auth/status','/api/tmdb-user/lists','/api/tmdb-user/lists/:id/items','/api/ai-status','/api/ai/news-analysis','/api/massive-status','/api/massive/ticker/:ticker','/api/massive/quote/:ticker','/api/massive/news','/api/massive/market-status','/api/engagement/:contentId','/api/engagement/:contentId/:action']}));
app.get('/health',(_req,res)=>res.json({ok:true,x_configured:Boolean(process.env.X_BEARER_TOKEN),youtube_configured:Boolean(process.env.YOUTUBE_API_KEY),watchmode_configured:watchmodeConfigured(),tmdb_configured:tmdbConfigured(),tmdb_lists_configured:Boolean(process.env.TMDB_API_READ_ACCESS_TOKEN),gemini_configured:geminiConfigured(),massive_configured:massiveConfigured(),x_username:X_USERNAME,authentication:getCredentialStatus(),timestamp:new Date().toISOString()}));
